...
from byd_undistortion import Undistortion
...
class UFLDv2:
    ...
    def forward(self, img_ori):
        ...
        return coords_ori_space, im0_draw, lane_fits_params, fitted_lane_coords
...
if __name__ == "__main__":
    ...
    # --- NEW: UDP Sender Setup ---
    UDP_IP = "192.168.1.200"
    UDP_PORT = 12345  # You can change this port
    
    try:
        udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # UDP
        print(f"--- UDP Sender Initialized: Sending to {UDP_IP}:{UDP_PORT} ---")
    ...
    frame_count = 0
    while True:
        success, img = undis.cap.read()
        if not success:
            print("Finished processing video.")
            break
        
        frame_count += 1
        frame_undis = undis.undistort_frame(img)
        # --- NEW: Updated forward call to get lane_fits ---
        coords_ori_space, im0_draw, lane_fits_params, fitted_lane_coords = isnet.forward(frame_undis.download())
        ...
        for lane_idx, lane_pts in fitted_lane_coords:
            if lane_idx == 1: # 1 is ego left lane for CuLane
                left_lane = lane_pts
                isnet.last_known_left_lane = lane_pts
            elif lane_idx == 2: # 2 is ego right lane for CuLane
                right_lane = lane_pts
        ...
        # --- NEW: Send offset_m over UDP ---
        if offset_m is not None and udp_socket is not None:
            try:
                # Format the float as a string (e.g., "0.1234")
                message = f"{offset_m:.4f}"
                # Send the message
                udp_socket.sendto(message.encode('utf-8'), (UDP_IP, UDP_PORT))
            except socket.error as e:
                # This might happen if the host is unreachable
