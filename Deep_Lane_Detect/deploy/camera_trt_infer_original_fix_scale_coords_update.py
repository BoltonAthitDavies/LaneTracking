...
# --- Undistortion Class (Kept from original) ---
class Undistortion():
    def __init__(self):
        self._init_cv()
    ...
    def _load_calibration(self):
        try:
            ...
            self.path_yaml = "/home/nvidia/LaneTracking/Stereo_Fisheye_Camera_to_Depth/camera_calibration.yaml"
    ...
    def undistort_frame(self, left_frame, width_img=480, height_img=320):
        """
        ...
        # Resize for display
        d_left_u_display = cv2.cuda.resize(d_left_u, (width_img, height_img))
        
        # Download the final image from GPU to CPU
        return d_left_u_display.download()
...
if __name__ == "__main__":
    ...
    train_width = isnet.input_width
    train_height = isnet.input_height
    undis = Undistortion()
    ...
    while True:
        ...
        img_undis = undis.undistort_frame(img, width_img=train_width, height_img=train_height)
        # --- NEW: Updated forward call to get lane_fits ---
        coords_ori_space, im0_draw, lane_fits_params, fitted_lane_coords, fps = isnet.forward(img_undis)
