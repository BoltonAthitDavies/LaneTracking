...
class UFLDv2:
    def __init__(self, engine_path, config_path, ori_size):
        ...
        self.row_anchor = np.linspace(0.42, 1, self.num_row) # original
    ...
    def pred2coords(self, pred):
        # อ่านขนาดของเทนเซอร์
        # รู้ว่าโมเดลแบ่งกริดกี่ช่อง, มี lane กี่เส้น
        batch_size, num_grid_row, num_cls_row, num_lane_row = pred['loc_row'].shape
        batch_size, num_grid_col, num_cls_col, num_lane_col = pred['loc_col'].shape

        # หาตำแหน่งที่มีความน่าจะเป็นสูงสุด
        # ใช้ argmax เพื่อหาจุดที่โมเดลมั่นใจมากที่สุด
        max_indices_row = pred['loc_row'].argmax(1) # x ตำแหน่งสูงสุดในแต่ละ row
        # n , num_cls, num_lanes
        valid_row = pred['exist_row'].argmax(1)     # จุดที่มีเลนใน row
        # n, num_cls, num_lanes
        max_indices_col = pred['loc_col'].argmax(1) # y ตำแหน่งสูงสุดในแต่ละ col
        # n , num_cls, num_lanes
        valid_col = pred['exist_col'].argmax(1)     # จุดที่มีเลนใน col
        # n, num_cls, num_lanes
        ...
        coords = []
        # กำหนด lane index
        row_lane_idx = [1, 2]   # เส้นกลาง ใช้ row-based
        col_lane_idx = [0, 3]   # เส้นซ้าย/ขวาสุด ใช้ col-based

        # คำนวณพิกัด lane จาก row-based
        for i in row_lane_idx:
            tmp = []
            if valid_row[0, :, i].sum() > num_cls_row / 2:
                # print(f"valid_row: {valid_row[0, :, i].sum()} > {num_cls_row / 2}")
                for k in range(valid_row.shape[1]):
                    if valid_row[0, k, i]:
                        # print(f"valid_row: {valid_row[0, k, i]}")
                ...
                coords.append(tmp)
        '''
        - เช็คว่าจุด valid เพียงพอ (มีเลนจริง)

        - หาช่วง index รอบตำแหน่งสูงสุด max_indices_row

        - ใช้ softmax ถ่วงน้ำหนัก เพื่อหาค่าเฉลี่ยตำแหน่ง (ละเอียดกว่าการใช้ argmax ตรง ๆ)

        - คูณด้วยความกว้างภาพจริง self.ori_img_w เพื่อได้ค่า x ในพิกัดจริง

        - y มาจาก row_anchor[k] * self.ori_img_h
        '''

        # คำนวณพิกัด lane จาก col-based (เหมือนกันแต่สลับ x/y)
        for i in col_lane_idx:
            tmp = []
            if valid_col[0, :, i].sum() > num_cls_col / 4:
                # print(f"valid_col: {valid_col[0, :, i].sum()} > {num_cls_col / 4}")
                for k in range(valid_col.shape[1]):
                    if valid_col[0, k, i]:
                        # print(f"valid_col: {valid_col[0, k, i]}")
...
# -----------------------------------------------------------------------
        ...
        print(f"Total lanes detected: {len(coords)}")
        for i, lane in enumerate(coords):
            print(f" Lane {i+1}: {len(lane)} points")
            if lane:  # if lane has points
                print(f"  First point: {lane[0]}")
        print("\n================================")
# -----------------------------------------------------------------------
        for lane in coords:
            for coord in lane:
        ...
        # print(self.row_anchor,self.col_anchor)
...
# python deploy/trt_infer.py --config_path configs/culane_res34.py --engine_path engine/culane_res34.engine --video_path example.mp4