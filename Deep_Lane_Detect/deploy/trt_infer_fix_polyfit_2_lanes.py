...
class UFLDv2:
    ...
    def forward(self, img):
        ...
        # --------------------------------------------------
        # --- Visualization ---
        lane_colors = {0: (0, 0, 255), 1: (0, 255, 0)}  # Left = Red, Right = Green
        ...
        for lane_id in [0, 1]:
            ...
            if len(pts) >= 3:
                ...
                try:
                    ...
                    y_new = np.linspace(min(y), max(y), num=72)
                    x_new = poly(y_new)
        ...
        for lane_id, data in lane_fits.items():
            if data["coeffs"] is not None:
                if lane_id == 0:
                    id_l = lane_id
                    left_lane = data['points'][:5]
                if lane_id == 1:
                    id_r = lane_id
                    right_lane = data['points'][:5]
                # print(f"Lane {lane_id} coeffs: {data['coeffs']}")
...
def get_args():
    ...
    parser.add_argument('--ori_size', default=(480, 320), help='size of original frame', type=tuple)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        resize_img = cv2.resize(img, args.ori_size)
        isnet.forward(resize_img)
