...
class UFLDv2:
    def __init__(self, engine_path, config_path, ori_size):
        ...
        self.row_anchor = np.linspace(0.42, 1, self.num_row) # for 1600x320
        # self.row_anchor = np.linspace(0.2, 1, self.num_row) # for 1280x720
...
def get_args():
    ...
    # parser.add_argument('--ori_size', default=(1920, 1080), help='size of original frame', type=tuple)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # h_orig, w_origi = img.shape[:2]
        # w_target, h_target = args.ori_size
        # scale = min(w_target/w_origi, h_target/h_orig)

        # new_w = int(w_origi * scale)
        ...
        # canvas[0:new_h, x_offset:x_offset + new_w] = scaled_image

        # # print(f"Final shape: {canvas.shape}")

        # isnet.forward(canvas)
        # =======================Original============================
