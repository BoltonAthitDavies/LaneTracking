...
def combine_tusimple_test(work_dir, exp_name):
    ...
    all_res = []
    for i in range(size):
        output_path = os.path.join(work_dir, f"{exp_name}.{i}.txt")
        with open(output_path, 'r') as fp:
            res = fp.readlines()
        all_res.extend(res)
    ...
    for res in all_res:
        try:
            ...
            raw_file = json_obj['raw_file']
            if raw_file not in names:
                names.add(raw_file)
                all_res_no_dup.append(res)
        except Exception as e:
            print(f"[combine_tusimple_test] Warning: could not parse line: {res}")
            continue

    output_path = os.path.join(work_dir, f"{exp_name}.txt")
    with open(output_path, 'w') as fp:
        fp.writelines(all_res_no_dup)
# ========================================================================================
