...
def generate_segmentation_and_train_list(root, line_txt, names):
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        label = np.zeros((240, 424),dtype=np.uint8)
        start, stop, step = 110, 240, 10
        the_anno_row_anchor = np.arange(start, stop, step) # start, stop, step
        all_points = np.zeros((4, len(the_anno_row_anchor), 2), dtype=np.float64)
        ...
        if len(k_neg) == 1:                                           # for only one lane in the left
            ...
            yy = ((np.array(lines[which_lane][1::2]) - start) / 10).astype(int)
...
if __name__ == "__main__":
    ...
    # training set
    names,line_txt = get_tusimple_list(args.root,  ['label_data_training.json'])
    # generate segmentation and training list for training
    generate_segmentation_and_train_list(args.root, line_txt, names)

    # testing set
    names,line_txt = get_tusimple_list(args.root, ['test_tasks.json'])
