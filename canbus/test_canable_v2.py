...
def create_command(can_id, velocity, steering):
...
def send_phase(bus, duration=10, velocity=2.0, steering=40.0):
    ...
    # Create messages
    move_msg = create_command(0x315, velocity, steering)
    stop_msg = create_command(0x315, 0.0, 0.0)
    ...
    while (time.time() - start_time) < duration:
        ...
        if msg:
            ...
            if int(elapsed) > last_print_time:
                ...
                if msg.arbitration_id == 0x915 and len(msg.data) == 8:
                    vel, steer = struct.unpack('>ff', msg.data)
                    print(f"         Last feedback Left ERPM: {vel:6.2f} m/s, {steer:6.2f}°")
                if msg.arbitration_id == 0x916 and len(msg.data) == 8:
                    vel, steer = struct.unpack('>ff', msg.data)
                    print(f"         Last feedback Right ERPM: {vel:6.2f} m/s, {steer:6.2f}°")
...
def send_receive_phase(bus, duration=10, velocity=2.0, steering=40.0):
    ...
    # Create messages
    move_msg = create_command(0x316 ,velocity, steering)
    stop_msg = create_command(0x316, 0.0, 0.01)
...
def main():
    ...
    try:
        ...
        # Send stop command
        try:
            stop_msg = create_command(0x202 ,0.0, 0.0)
            for _ in range(10):
                bus.send(stop_msg)
                time.sleep(0.01)
