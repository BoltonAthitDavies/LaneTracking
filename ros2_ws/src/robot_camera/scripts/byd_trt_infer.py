...
class UFLDv2:
    ...
    #     # <-- MODIFIED: Get all 3 return values -->
    #     im0_draw, lane_fits_params, fitted_lane_coords = self.fit_and_draw_lanes(coords_model_space, im0, fps)
        
    #     # <-- MODIFIED: Return all 4 values -->
    #     return coords_model_space, im0_draw, lane_fits_params, fitted_lane_coords
    
    #     # ----------------------------------

    def forward(self, img_gpumat): # <--- Renamed
        start_time = time.time()

        # --- THIS IS THE FIX ---
        # Download from GpuMat *once*
        # This is the "correct" BGR data
        im0 = img_gpumat.download()
        # --- END FIX ---

        # --- This is your original preprocessing code ---
        img = im0[self.cut_height:, :, :] # Crop
        img = cv2.resize(img, (self.input_width, self.input_height), cv2.INTER_CUBIC)
        img = img.astype(np.float32) / 255.0
        img = np.transpose(np.float32(img[:, :, :, np.newaxis]), (3, 2, 0, 1))
        img = np.ascontiguousarray(img)
        # --- End original preprocessing ---
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # <-- MODIFIED: Get all 4 return values -->
        coords_ori_space, im0_draw, lane_fits_params, fitted_lane_coords = isnet.forward(img)
        
        # You can now use fitted_lane_coords, which is [(idx, fitted_points), ...]
