...
class LaneDetectUFLD(Node):
    def __init__(self):
        ...
        # self.create_subscription(CompressedImage, "/cam0_undis/image_raw/compressed", self.cam_callback, qos)
        # self.create_subscription(CompressedImage, "/depth/image_raw", self.depth_callback, qos)
    ...
    def depth_callback(self, msg):
        try:
            # compress_depth = self.get_parameter('compress_depth').value
            # depth = self.bridge.compressed_imgmsg_to_cv2(msg, "passthrough")
    ...
    def pixels_to_camera_lane(self, lane_points, depth_img):
        ...
        # Get depth values
        Zs = np.array([depth_img[v, u] for u, v in zip(us, vs)], dtype=np.float32)

        # Find invalid indices (0, nan, inf)
        invalid = (Zs <= 0) | np.isnan(Zs) | np.isinf(Zs)
    ...
    # === Publish lane path ===
    def publish_lane_path(self, lane_points, frame_id, pub):
        """
    ...
    def publish_lane_path_cam(self, lane_points, pub, frame_id="cam0_undis"):
        """
        lane_points: Nx3 numpy array in camera frame
        pub: ROS2 Path publisher
    ...
    #     # Define pitch rotation (25 degrees)
    #     pitch_deg = -25.0
    ...
    #     for X, Y, Z in lane_points:
    #         # Apply axis reordering first
    #         cam_point = np.array([X, Y, Z])  # your original mapping

    #         # Apply pitch rotation
    #         rotated_point = R_pitch @ cam_point

    #         pose = PoseStamped()
    #         pose.header = path_msg.header
    #         pose.pose.position.x = float(rotated_point[0])
    #         pose.pose.position.y = float(rotated_point[1])
    #         pose.pose.position.z = float(rotated_point[2])
    #         pose.pose.orientation.w = 1.0  # identity quaternion
    #         path_msg.poses.append(pose)
    ...
    def compute_center_lane(self, im0, left_lane, right_lane, lane_width):
        """
    ...
    def run_lane_detection(self, frame):
        ...
        im0, centers = self.compute_center_lane(im0, left_lane, right_lane, lane_width=0.42)
        # Show result
        if show_images:
            cv2.imshow("result", im0)
            cv2.waitKey(1)
        # print(f"Left lane coords {id_l}: {left_lane}")
        # print(f"Right lane coords {id_r}: {right_lane}")
        if self.latest_depth is not None:
            # Pixel → camera coordinates
            left_lane_cam = self.pixels_to_camera_lane(left_lane, self.latest_depth)
            right_lane_cam = self.pixels_to_camera_lane(right_lane, self.latest_depth)
            center_lane_cam = self.pixels_to_camera_lane(centers, self.latest_depth)

            # Camera → base_footprint with Z=0
            left_lane_base = self.camera_to_base_batch(left_lane_cam, force_z0=True)
            right_lane_base = self.camera_to_base_batch(right_lane_cam, force_z0=True)
            center_lane_base = self.camera_to_base_batch(center_lane_cam, force_z0=True)

            # Publish to ROS2
            self.publish_lane_path(left_lane_base, 'base_footprint', self.left_lane_pub)
            self.publish_lane_path(right_lane_base, 'base_footprint', self.right_lane_pub)
            self.publish_lane_path(center_lane_base, 'base_footprint', self.center_lane_pub)
            # Publish in camera frame
            self.publish_lane_path_cam(left_lane_cam, self.left_lane_cam_pub, frame_id='cam0_undis')
            self.publish_lane_path_cam(right_lane_cam, self.right_lane_cam_pub, frame_id='cam0_undis')
            # print("Left lane in base frame:\n", left_lane_base)
            # print("Right lane in base frame:\n", right_lane_base)
            # self.compute_and_publish_center_lane(left_lane_base, right_lane_base, self.center_lane_pub, frame_id="base_footprint", lane_width=0.42)
